...
function user_setup()
    state.OffenseMode:options('Normal', 'LightAcc', 'Acc', 'Fodder', 'Skadi', 'Thaumas')
	...
	send_command('bind F11 exec rudras_bully_sa.txt')
	send_command('bind F10 exec rudras_sa.txt')
	send_command('bind F9 ws Rudras Storm <t>')
...
function init_gear_sets()
    ...
    sets.precast.JA['Steal'] = {head="Plunderer's Bonnet",hands="Pill. Armlets +1",legs="Pill. Culottes +1",feet="Pillager's Poulaines +1"}
	...
	sets.precast.WS['Shark Bite'].SA = set_combine(sets.precast.WS['Shark Bite'].Mod, {body="Pillager's Vest +1",legs="Pill. Culottes +1"})
	sets.precast.WS['Shark Bite'].TA = set_combine(sets.precast.WS['Shark Bite'].Mod, {body="Pillager's Vest +1",legs="Pill. Culottes +1"})
	sets.precast.WS['Shark Bite'].SATA = set_combine(sets.precast.WS['Shark Bite'].Mod, {body="Pillager's Vest +1",legs="Pill. Culottes +1"})
	...
	sets.precast.WS['Mandalic Stab'].SA = set_combine(sets.precast.WS['Mandalic Stab'].Mod, {body="Pillager's Vest +1",legs="Pill. Culottes +1"})
	sets.precast.WS['Mandalic Stab'].TA = set_combine(sets.precast.WS['Mandalic Stab'].Mod, {body="Pillager's Vest +1",legs="Pill. Culottes +1"})
	sets.precast.WS['Mandalic Stab'].SATA = set_combine(sets.precast.WS['Mandalic Stab'].Mod, {body="Pillager's Vest +1",legs="Pill. Culottes +1"})

	sets.precast.WS['Aeolian Edge'] = {{hands="Plun. Armlets +1", waist="Chaac Belt", feet="Raider's Poulaines +2"}}
    ...
    sets.midcast.FastRecast = {
        ...
        body="Pillager's Vest +1",hands="Pill. Armlets +1",
    ...
    sets.midcast.Utsusemi = {
        ...
        body="Pillager's Vest +1",hands="Pill. Armlets +1",ring1="Beeline Ring",
        back="Canny Cape",legs="Kaabnax Trousers",feet="Iuitl Gaiters +1"}
	...
	sets.idle.Weak = {ammo="Ginsen",
		...
		back="Shadow Mantle",waist="Flume Belt",legs="Pill. Culottes +1",feet="Skadi's Jambeaux +1"}
	...
	sets.engaged.Acc.Evasion = {ammo="Ginsen",
...
if player.tp > 1000 and player.status == "Engaged" then
    send_command('ws Rudras Storm <t>')
	end	
end--]]
...
--[[function job_self_command(cmdParams, eventArgs)
    if cmdParams[1]:lower() == 'lockrange' then
        if LockRange == 'ON' then
			LockRange = 'OFF'							
			add_to_chat(57,'Lock Range/Ammo: Disabled')
		else
			LockRange = 'ON'							
			add_to_chat(158,'Lock Range/Ammo: Enabled')					
		end
	end    
end--]]
